...
- **No API keys required** for live *and* historical market data — `/markets`, `/events`, `/markets/candlesticks`, `/markets/trades`, `/historical/{cutoff,markets,trades}` and single orderbooks are all public. If you set `KALSHI_ACCESS_KEY` + `KALSHI_PRIVATE_KEY_PEM` the client also signs requests (needed for `/markets/orderbooks` batch and account endpoints); otherwise it logs and continues unsigned.
